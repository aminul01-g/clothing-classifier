...
def get_class_image(idx):
    filename = f"class_{idx}.png"
    ...
    draw.text(((28-w)/2, (28-h)/2), text, fill=0, font=font)
    return img

cols = st.columns(5)
for i, class_name in enumerate(CLASS_NAMES):
    with cols[i % 5]:
        st.image(get_class_image(i), width=56, caption=f"{i}: {class_name}")
...
if uploaded_file is not None:
    ...
    with torch.no_grad():
        ...
        st.image(get_class_image(pred_class), width=56, caption=f"Predicted: {CLASS_NAMES[pred_class]}") 